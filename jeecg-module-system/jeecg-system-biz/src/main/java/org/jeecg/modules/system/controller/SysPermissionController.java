...
import org.jeecg.config.shiro.CurrentUser;
...
public class SysPermissionController {
	...
	public Result<List<SysPermissionTree>> list(SysPermission sysPermission, HttpServletRequest req,@RequestHeader(required = false) String frontMenu) {
		long start = System.currentTimeMillis();
		String s = sysPermission.getFrontMenu();
		if(StringUtils.isEmpty(s)){
			sysPermission.setFrontMenu(frontMenu);
		}
		Result<List<SysPermissionTree>> result = new Result<>();
		try {
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.eq(StringUtils.isNotEmpty(s),SysPermission::getFrontMenu, s);
	...
	public Result<List<SysPermissionTree>> getSystemMenuList(@RequestHeader(required = false) String frontMenu) {
		long start = System.currentTimeMillis();
		Result<List<SysPermissionTree>> result = new Result<>();
		try {
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
			...
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.eq(StringUtils.isNotEmpty(frontMenu),SysPermission::getFrontMenu,frontMenu);
			query.orderByAsc(SysPermission::getSortNo);
			List<SysPermission> list = sysPermissionService.list(query);
	...
	public Result<List<SysPermissionTree>> getSystemSubmenu(@RequestParam("parentId") String parentId,@RequestHeader(required = false) String frontMenu){
		Result<List<SysPermissionTree>> result = new Result<>();
		try{
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
	...
	public Result getSystemSubmenuBatch(@RequestParam("parentIds") String parentIds,@RequestHeader(required = false) String frontMenu) {
		try {
...
//	}
	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request,@RequestHeader(required = false) String frontMenu) {
		...
		try {
			...
			if(StringUtils.isNotEmpty(frontMenu)){
				metaList=metaList.stream().filter(sysPermission -> frontMenu.equals(sysPermission.getFrontMenu())).collect(Collectors.toList());
			}
			//添加首页路由
	...
	@AutoLog(value = "菜单添加",operateType = CommonConstant.OPERATE_TYPE_2)
	...
	@AutoLog(value = "菜单编辑",operateType = CommonConstant.OPERATE_TYPE_3)
	...
	@AutoLog(value = "菜单删除",operateType = CommonConstant.OPERATE_TYPE_4)
	...
	@AutoLog(value = "菜单批量删除",operateType = CommonConstant.OPERATE_TYPE_4)
