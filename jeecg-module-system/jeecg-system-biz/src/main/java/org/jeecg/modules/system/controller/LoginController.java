...
public class LoginController {
	...
	//纯数字验证码，去除掉字母大小写
	private final String BASE_CHECK_CODES = "1234567890";
	//private final String BASE_CHECK_CODES = "qwertyuiplkjhgfdsazxcvbnmQWERTYUPLKJHGFDSAZXCVBNM1234567890";

	private Result<JSONObject> pcLogin(SysLoginModel sysLoginModel,Boolean isUseCaptcha,Boolean isEncryptPwd) {
		Result<JSONObject> result = new Result<JSONObject>();
		String username = sysLoginModel.getUsername();
		String password = sysLoginModel.getPassword();
		//update-begin-author:taoyan date:2022-11-7 for: issues/4109 平台用户登录失败锁定用户
		if(isLoginFailOvertimes(username)){
			return result.error500("该用户登录失败次数过多，已被锁定，请于10分钟后再次登录！");
		}
		...
		//先将密码通过sm2私钥解密
		String desEncryptPwd = password;
        try {
			if (isEncryptPwd){
				desEncryptPwd = SM2EncryptUtil.decrypt(password);
			}
        ...
        //2. 校验用户名或密码是否正确
		String userpassword = PasswordUtil.encrypt(username, desEncryptPwd, sysUser.getSalt());
		String syspassword = sysUser.getPassword();
		if (!syspassword.equals(userpassword)) {
			//update-begin-author:taoyan date:2022-11-7 for: issues/4109 平台用户登录失败锁定用户
			String errorMsg = addLoginFailOvertimes(username);
			//update-end-author:taoyan date:2022-11-7 for: issues/4109 平台用户登录失败锁定用户
		...
		if (isUseCaptcha && StrUtil.isNotBlank(realKey)){
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel){
		return pcLogin(sysLoginModel,true,true);
	}

	@ApiOperation("账号密码登陆，无需验证码")
	@RequestMapping(value = "/accountLogin", method = RequestMethod.POST)
	public Result<JSONObject> accountLogin(@RequestBody SysLoginModel sysLoginModel){
		return pcLogin(sysLoginModel,false,false);
	}
	 ...
	 * @param username
	 */
	private String addLoginFailOvertimes(String username){
		String key = CommonConstant.LOGIN_FAIL + username;
		Object failTime = redisUtil.get(key);
		Integer val = 0;
		if(failTime!=null){
		...
		String errorMsg = "";
		if (val >= 5){
			errorMsg = String.format("账号密码已连续错误%d次,将被锁定%d分钟,请稍后登录",val,10);
		}else if (val>=3){
			errorMsg = String.format("账号密码已连续错误%d次,错误超过%d次，将被锁定%d分钟",val,5,10);
		}else {
			errorMsg = "用户名或密码错误";
		}
		return errorMsg;
