...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	public List<SysPermissionDataRuleModel> queryPermissionDataRule(String component, String requestPath, String username) {
		// TODO 此处需要根据component过滤数据，如果按钮权限的component字段为空，那么定义的数据权限规则不会生效；
		...
		if(oConvertUtils.isNotEmpty(component)) {
			...
			List<SysPermission> temp=new ArrayList<SysPermission>();
			currentSyspermission.forEach(i->{
				String id = i.getId();
				List<SysPermission> sysPermissions = sysPermissionMapper.selectList(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getParentId, id));
				temp.addAll(sysPermissions);
			});
			currentSyspermission.addAll(temp);
		}else {
			//1.直接通过前端请求地址查询菜单
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
