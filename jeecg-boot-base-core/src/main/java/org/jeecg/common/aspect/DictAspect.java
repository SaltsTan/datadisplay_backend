...
public class DictAspect {
        ...
        if (result instanceof Result) {
            if (((Result) result).getResult() instanceof IPage) {
                ...
                for (Object record : records) {
                    getDictFileds(items, dictFieldList, dataListMap, record);
                }
                ...
                for (JSONObject record : items) {
                    translateItems(dictFieldList, translText, record);
                }
                ...
                for (Object record : (List) ((Result) result).getResult()) {
                    if(ObjectUtil.isBasicType(record)||record instanceof String){
                        return result;
                    }
                    getDictFileds(items, dictFieldList, dataListMap,record);
                }
                ...
                if(ObjectUtil.isBasicType(obj)||obj instanceof String){
                    return result;
                }
                List<JSONObject> items = new ArrayList<>();
                //step.1 筛选出加了 Dict 注解的字段列表
                ...
                getDictFileds(items, dictFieldList, dataListMap,obj);

                if(items.isEmpty()){
                    return result;
                }
                Map<String, List<DictModel>> translText = this.translateAllDict(dataListMap);
                translateItems(dictFieldList,translText, items.get(0));
    ...
    private void translateItems(List<Field> dictFieldList, Map<String, List<DictModel>> translText, JSONObject record) {
    ...
    private void getDictFileds(List<JSONObject> items, List<Field> dictFieldList, Map<String, List<String>> dataListMap, Object record) {
        String json="{}";
        try {
